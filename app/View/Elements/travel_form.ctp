...
if(empty ($this->request->data)) $saveButtonText = 'Crear';
else $saveButtonText = 'Salvar';
?>

<div>
    <div id='travel-ajax-message'></div>
    <?php
    echo $this->Form->create('Travel', array('default'=>!$do_ajax, 'url'=>array('controller'=>'travels', 'action'=>$form_action), 'style'=>$style));
    ?>
    <fieldset>
    <?php
    ...
    if($is_modal) echo $this->Form->button(__('Cancelar'), array('id'=>'btn-cancel-travel', 'style'=>'display:inline-block'));
    ?>
    </fieldset>
...
//$this->Html->script('datepickers', array('inline' => false));
